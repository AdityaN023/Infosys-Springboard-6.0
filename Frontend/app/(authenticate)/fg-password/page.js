...
const Forgot_Password = () => {
    ...
    const onSubmit = async (data) => {
        if (verification) {
            ...
            if (result) {
                const updateUser = await fetch('http://localhost:3000/api/signUp', {
                    method: 'PUT',
                    body: JSON.stringify({ email: data.email, password: data.password })
                });
                const done = await updateUser.json().then((result) => result.done);
                if (done) {
            ...
            const checkUserExist = await fetch('http://localhost:3000/api/signUp?email=' + data.email);
            const userExist = await checkUserExist.json().then((result) => result.userExist);
            if (userExist) {
                ...
                router.push('/fg-password?userExist=false')
            }
        }
    }
    ...
    return (
        <>
            <h2 className="font-bold font-sans text-3xl text-center">Forgot Password</h2>
            <form className="space-y-4 md:space-y-6" onSubmit={handleSubmit(onSubmit)}>
                {
                    searchParams.has('userExist', false)
                        ? <div className="flex items-start sm:items-center p-4 mb-4 text-sm bg-red-800 rounded-md text-red-200 font-semibold" role="alert">
                            ...
                            <button type='button' className="ml-auto cursor-pointer" onClick={() => router.push('/fg-password')}>
                        ...
                        ? <div className='space-y-4 md:space-y-6'>
                            <div className='md:space-y-2'>
                                ...
                                <p className='font-sans text-center'>
                                    Please type the Verification code sent to <span>aditya.nachanekar023@gmail.com</span>
                                </p>
                            </div>
                            {
                                error
                            ...
                            <p className="text-sm font-light text-orange-400 dark:text-orange-300 text-center">
                                Don't receive the verification code?
                                {
                                    counter === 0
                                        ...
                                        : `Resend in 00:${String(counter).padStart(2, '0')}`
                                }
                            </p>
                ...
                <p className="text-sm font-light text-gray-500 dark:text-gray-400">
                    <AnimeLink href="/" className="font-medium text-primary-600 hover:underline dark:text-primary-500">Back to Sign In</AnimeLink>
                </p>
            </form>
        </>
    )
