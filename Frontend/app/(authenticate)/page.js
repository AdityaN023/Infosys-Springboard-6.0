...
export default function Home() {
    ...
    const onSubmit = async (data) => {
        const res = await signIn('credentials', { email: data.email, password: data.password, remember: data.remember, redirect: false });
        if (res?.error) {
            setError(res.error);
        } else {
    ...
    return (
        <>
            ...
            {
                !error && searchParams.has('signOut', true) || searchParams.has('userExist', true) || searchParams.has('success', true) || searchParams.has('update', true)
